...
/* Bluetooth Parameter ---------------------------------------------------------------- */
/* Nano33BLE_A
BLEService customService("19B10000-E8F2-537E-4F6C-D00000000000");

BLEStringCharacteristic RawDataCharacteristic("19B10001-E8F2-537E-4F6C-D00000000001", BLERead | BLEWrite, 20);
BLEStringCharacteristic GaitCharacteristic("19B10001-E8F2-537E-4F6C-D00000000002", BLERead | BLEWrite, 20); //For Gait
BLEStringCharacteristic BatValCharacteristic("19B10001-E8F2-537E-4F6C-D00000000003", BLERead | BLEWrite, 20);
*/
...
BLEStringCharacteristic RawDataCharacteristic("19B10002-E8F2-537E-4F6C-D00000000001", BLERead | BLEWrite, 20);
BLEStringCharacteristic GaitCharacteristic("19B10002-E8F2-537E-4F6C-D00000000002", BLERead | BLEWrite, 20); //For Gait
BLEStringCharacteristic BatValCharacteristic("19B10002-E8F2-537E-4F6C-D00000000003", BLERead | BLEWrite, 20);
...
float accRawData[3] = {0.0, 0.0, 0.0};
...
void setup()
{
  ...
  /* Nano33BLE_A
  BLE.setDeviceName("Nano33BLE_A");
  BLE.setLocalName("Nano33BLE_A");
  */

  /* Nano33BLE_B
  
  */
  BLE.setDeviceName("Nano33BLE_B");
  BLE.setLocalName("Nano33BLE_B");

  BLE.setAdvertisedService(customService);
  customService.addCharacteristic(GaitCharacteristic);
  customService.addCharacteristic(RawDataCharacteristic);
  customService.addCharacteristic(BatValCharacteristic);
  BLE.addService(customService);
  BLE.advertise();  // 讓 A 板可以發現 B 板
}
...
void loop()
{
  ...
  */
  ...
  // Read Battery Value
  int batRawValue = analogRead(A7);
  float batVoltage = batRawValue * (2.1 / 645);
  int batPercent = volToPercent(batVoltage, 1.6, 2.1);
  ...
  */
  ...
  {
    flag = result.classification[ix].value;
    if(maximumConfidence < flag)
    {
      maximumConfidence = flag;
      gaitStatus = ix;
    }
  ...
  if(central.connected())
  {
    ...
    /*
      sprintf(rawData, "%.2f,%.2f,%.2f", accRawData[0], accRawData[1], accRawData[2]);
      RawDataCharacteristic.writeValue((String)rawData);  //Acc Raw Data
    */
    sprintf(rawData, "%.2f", accRawData[2]);  //Send Z axes raw data
    RawDataCharacteristic.writeValue((String)rawData);

    GaitCharacteristic.writeValue((String)result.classification[gaitStatus].label); //Gait Data

    BatValCharacteristic.writeValue((String)batPercent);
  }
...
int volToPercent(float x, float in_min, float in_max) {
  float mapped = (x - in_min) * 100 / (in_max - in_min);

  return constrain((int)mapped, 0, 100);
}
